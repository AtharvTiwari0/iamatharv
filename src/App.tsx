...
export default function App() {
  ...
  return (
    <div className="relative w-full min-h-screen text-ink-200 antialiased overflow-clip">
      ...
      <header className={`fixed left-0 right-0 top-4 z-50 flex justify-center px-4 md:top-5 pointer-events-none transition-all duration-500 ${showMiniHeader ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 -translate-y-4'}`}>
        <a href="#top" onClick={(e) => scrollTo(e, 'top')} aria-label="Back to top"
          className="group inline-flex min-h-[44px] items-center gap-3 rounded-full border border-ink-200/18 bg-ink-800/72 px-5 py-2 shadow-lg backdrop-blur-2xl hover:border-peach-300/45 text-cream-100 transition-colors">
